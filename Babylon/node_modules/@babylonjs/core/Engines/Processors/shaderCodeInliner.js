...
var ShaderCodeInliner = /** @class */ (function () {
    ...
    ShaderCodeInliner.prototype._collectFunctions = function () {
        ...
        while (startIndex < this._sourceCode.length) {
            ...
            var funcParamsEndIndex = CodeStringParsingTools.ExtractBetweenMarkers('(', ')', this._sourceCode, funcParamsStartIndex);
            ...
            var funcBodyStartIndex = CodeStringParsingTools.SkipWhitespaces(this._sourceCode, funcParamsEndIndex + 1);
            ...
            var funcBodyEndIndex = CodeStringParsingTools.ExtractBetweenMarkers('{', '}', this._sourceCode, funcBodyStartIndex);
            ...
            var params = CodeStringParsingTools.RemoveComments(funcParams).split(",");
            var paramNames = [];
    ...
    ShaderCodeInliner.prototype._replaceFunctionCallsByCode = function () {
        ...
        for (var _i = 0, _a = this._functionDescr; _i < _a.length; _i++) {
            ...
            while (startIndex < this._sourceCode.length) {
                ...
                var callParamsStartIndex = CodeStringParsingTools.SkipWhitespaces(this._sourceCode, functionCallIndex + name_1.length);
                if (callParamsStartIndex === this._sourceCode.length || this._sourceCode.charAt(callParamsStartIndex) !== '(') {
                    startIndex = functionCallIndex + name_1.length;
                    continue;
                }
                // extract the parameters of the function call as a whole string (without the leading / trailing parenthesis)
                var callParamsEndIndex = CodeStringParsingTools.ExtractBetweenMarkers('(', ')', this._sourceCode, callParamsStartIndex);
                ...
                var splitParameterCall = function (s) {
                    ...
                    while (curIdx < s.length) {
                        if (s.charAt(curIdx) === '(') {
                            var idx2 = CodeStringParsingTools.ExtractBetweenMarkers('(', ')', s, curIdx);
                            if (idx2 < 0) {
                                return null;
                            }
                            curIdx = idx2;
                ...
                var params = splitParameterCall(CodeStringParsingTools.RemoveComments(callParams));
                if (params === null) {
                    if (this.debug) {
                ...
                if (retParamName) {
                    ...
                    var injectDeclarationIndex = CodeStringParsingTools.FindBackward(this._sourceCode, functionCallIndex - 1, '\n');
    ...
    ShaderCodeInliner.prototype._replaceNames = function (code, sources, destinations) {
        var _loop_1 = function (i) {
            var source = new RegExp(CodeStringParsingTools.EscapeRegExp(sources[i]), 'g'), sourceLen = sources[i].length, destination = destinations[i];
